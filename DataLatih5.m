...
% menetapkan target_latih
    ...
    for k = 1:60
        target_latih{k} = 'Setengah Matang';
    end
...
Mdl2 = fitcecoc(data_latih, target_latih);

% Melakukan pelatihan menggunakan algoritma MSVM dengan kernel linear
Mdl_linear2 = fitcecoc(data_latih, target_latih, 'Learners', templateSVM('KernelFunction', 'linear'));

% Melakukan pelatihan menggunakan algoritma MSVM dengan kernel polynomial
Mdl_poly2 = fitcecoc(data_latih, target_latih, 'Learners', templateSVM('KernelFunction', 'polynomial'));

% Melakukan pelatihan menggunakan algoritma MSVM dengan kernel RBF (Gaussian)
Mdl_rbf2 = fitcecoc(data_latih, target_latih, 'Learners', templateSVM('KernelFunction', 'rbf'));


% membaca kelas keluaran hasil pelatihan
kelas_keluaran = predict (Mdl2,data_latih);

kelas_keluaranlinear = predict (Mdl_linear2, data_latih);

kelas_keluaranpoly = predict (Mdl_poly2, data_latih);

kelas_keluaranrbf = predict (Mdl_rbf2, data_latih);
...
save Mdl2 Mdl2
save Mdl_linear2 Mdl_linear2 
save Mdl_poly2 Mdl_poly2
save Mdl_rbf2 Mdl_rbf2