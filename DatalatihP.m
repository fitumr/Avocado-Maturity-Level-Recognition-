...
morfo = bwareaopen(imcomp,2000);

% Konversi ke tipe data double
img = double(morfo);

% Operator Prewitt
prewitt_mask1 = [-1 -1 -1; 0 0 0; 1 1 1];
prewitt_mask2 = [-1 0 1; -1 0 1; -1 0 1];

% Konvolusi dengan masing-masing kernel
prewitt_edge1 = conv2(img, prewitt_mask1, 'same');
prewitt_edge2 = conv2(img, prewitt_mask2, 'same');

% Gabungkan kedua hasil
prewitt_edge = sqrt(prewitt_edge1.^2 + prewitt_edge2.^2);

 % Ekstraksi fitur bentuk
    stats = regionprops(prewitt_edge, 'Area', 'Perimeter', 'Solidity', 'Eccentricity');
